...
public interface ModerateMapper {
    ...
    int insertMultiple(MultiRowInsertStatementProvider<Moderate> multipleInsertStatement);
    ...
    @Generated(value="org.mybatis.generator.api.MyBatisGenerator", date="2021-12-27T16:19:13.6828025+08:00", comments="Source Table: community.moderate")
    default int insertMultiple(Collection<Moderate> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, moderate, c ->
            c.map(id).toProperty("id")
            .map(uid).toProperty("uid")
            .map(communityId).toProperty("communityId")
            .map(createTime).toProperty("createTime")
